...
TaskNG.prototype.add = function(message){
  fs.readFile(process.env.HOME + "/.tasks", "utf-8", function(err, data){
    if (err){
      ...
      console.log("task before writing " + JSON.stringify(task));
      fs.unlink(process.env.HOME + "/.tasks", function(err){
        fs.writeFile(process.env.HOME + "/.tasks", JSON.stringify(task), "utf-8", function(err){
          if (err) throw err;
          console.log("task after writing " + JSON.stringify(task));
        });
      });
    }
  });
...
TaskNG.prototype.show = function(label){
  fs.readFile(process.env.HOME + "/.tasks", "utf-8", function(err, data){
    if (err) {
      fs.open(process.env.HOME + "/.tasks", "w", function(err, fd){
        if (err) throw err;
      ...
      for (var i=0; i < task.tasks.length; i++){
        var txt = "#" + (i + 1) + " : " + task.tasks[i];
        if (task.tasks[i].search(/:P1/i) >= 0 && task.tasks[i].search(":" + label) >= 0) {
          console.log(txt.red);
          continue;
        }
        if (task.tasks[i].search(/:P2/i) >= 0 && task.tasks[i].search(":" + label) >= 0) {
          console.log(txt.yellow);
          continue;
        } 
        if (task.tasks[i].search(":" + label) >= 0){
          console.log(txt);
        }
      } 
    }
...
TaskNG.prototype.edit = function(index, message){
  this.pop(index, this.add(message));
  
};

TaskNG.prototype.pop = function(index, callback){
  fs.readFile(process.env.HOME + "/.tasks", "utf-8", function(err, data){
    if (err) throw err;
    var task = JSON.parse(data);
